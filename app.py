...
# -----------------------------------------------------------------------------
    ...
    # --- TAB 3: YENİ SORU EKLEME FORMU ---
    with tab_t3:
        st.subheader("📝 Havuza Yeni Soru Ekleyin")
        st.caption("Eklenecek sorular doğrudan Supabase 'flashcards' tablosuna yazılır.")

        with st.form("add_card_form", clear_on_submit=True):
            col_s, col_t = st.columns(2)
            with col_s:
                input_subject = st.selectbox("Ders", ["Fizik", "Matematik", "Geometri", "Fen Bilgisi"])
            with col_t:
                input_topic = st.text_input("Konu Adı", placeholder="Örn: Vektörler, Trigonometri...")

            input_question = st.text_area("Soru Metni / Kavram (Term)", placeholder="Soruyu buraya yazın...")
            input_correct = st.text_input("✅ Doğru Cevap (Definition)")
            ...
            if submit_btn:
                if input_topic and input_question and input_correct:
                    try:
                        payload = {
                            "subject": input_subject,
                            "topic": input_topic.strip(),
                            "term": input_question.strip(),
                            "definition": input_correct.strip(),
                        ...
                        st.success(f"🎉 '{input_topic}' konusuna ait soru başarıyla havuza eklendi!")
                    except Exception as e:
                        st.error(f"Soru eklenirken hata oluştu: {e}")
                else:
                    st.warning("Lütfen Konu, Soru ve Doğru Cevap alanlarını eksiksiz doldurun.")
